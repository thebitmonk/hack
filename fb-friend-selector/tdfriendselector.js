...
var TDFriendSelector = (function(module, $) {
	...
	// Private variables
	settings, friends,
	$friends, $container, $friendsContainer, $searchField, $selectedCount, $selectedCountMax, $pageNumber, $pageNumberTotal, $pagePrev, $pageNext, $buttonClose, $buttonOK, $close, 
	...
	init = function(options) {
        ...
        $close            = $container.find(settings.closeClass);
        $fbpost           = $container.find(settings.fbpostSelector);
	};
	...
	newInstance = function(options) {
		...
		filterFriends = function(filter) {
            console.log('Filter friends ' + filter);
			var i, len;
			numFilteredFriends = 0;
		...
		bindEvents = function() {
            ...
            $fbpost.live('click', function(e){
                var wall_post = $('#fb_post_field').val();
                FB_POST = { message: wall_post, friend_ids: selectedFriendIds,};
                var url = 'http://192.168.1.127:8000/fb_wallpost/';
                $.ajax({
                    url: url,
                    type: 'POST',
                    data: JSON.stringify(FB_POST),
            ...
            $close.live('click', function(e){
                ...
                var friend_element = $('a[data-id="' + parent_id + '"]');
                selectFriend(friend_element);
            });
		...
		selectFriend = function($friend) {
            ...
            var friend_name = $friend.find('.TDFriendSelector_friendName').find('span')[0].textContent;
			...
			if (!$friend.hasClass(settings.friendSelectedClass)) {
				...
				if (selectedFriendIds.length < instanceSettings.maxSelection) {
					...
					if ($.inArray(friendId, selectedFriendIds) === -1) {
                        ...
                        try{
                            if($selectedCount.find('#' + friendId).size() == 0)
                            {
                                var selected_friend_names = $selectedCount.html();
                                friend_name = friend_name.split(' ')[0];
                                $selectedCount.html(selected_friend_names + '<span id="'+ friendId + '" class="friend_tags">' + friend_name + '<div class="close"></div></span>');
                            }
                        }catch(e){
                            console.log('Error occurred while trying to retrieve element');
                        }
